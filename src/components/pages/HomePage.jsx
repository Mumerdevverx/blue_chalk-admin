...
export default function HomePage() {
  ...
  const totalImagePages = Math.ceil(imageItems.length / imagesPerPage) || 1
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 p-6">
      <div className="w-full max-w-6xl mx-auto">
        ...
        {/* ===== ADD VIDEO EMPTY CARD (centered, only if no video) ===== */}
        {!loading && !hasVideo && (
  <div
  ...
  >
    <div className="flex flex-col items-center justify-center gap-3">
      ...
      <span className="text-2xl font-bold text-slate-700 dark:text-slate-300">Add Video</span>
      <span className="text-sm text-slate-500 dark:text-slate-400">Click to add your first video</span>
    </div>
  </div>
)}
        ...
        {/* Empty State (no items at all) – only if no video and no images? */}
        {!loading && items.length === 0 && (
          // This case is covered by the "Add Video" card above, but keep it for completeness
          // Actually we can skip this because the Add Video card already shows.
          // We'll remove this block to avoid duplication.
          // But if items.length === 0, the Add Video card shows, and the images section will show "No images yet".
          // So we can remove the empty state block entirely.
          null
        )}
        ...
        {/* ===== ADD IMAGE BUTTON – outside images card, right-aligned ===== */}
        {!loading && (
          ...
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl p-6 rounded-3xl shadow-xl border border-white/20 dark:border-slate-700/50">
            ...
            {imageItems.length === 0 ? (
              <div className="text-center py-12 text-slate-500 dark:text-slate-400">
                ...
                <p className="text-sm">No images yet. Click the <strong>Add Image</strong> button above to get started.</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {paginatedImages.map((item, index) => {
                    const isLatest = item._id === imageItems[imageItems.length - 1]?._id
                    return (
                      <div
